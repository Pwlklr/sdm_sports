...
from src.core.tournament.phase_state import BracketPhaseState, RoundRobinPhaseState
...
class TournamentPolicy(ABC):
    ...
    @abstractmethod
    def build_phases(
        self, blueprint: TournamentBlueprint
    ) -> tuple[Phase, ...]:
        pass
    ...
    def decide_perform_draw(
        ...
        **kwargs: Any,
        ...
        for slot in ps.slots:
            if slot.round_index == ps.current_round_index - 1 and slot.winner_id:
...
class MatchOutcomePolicyMixin(TournamentPolicy):
    _own_command_handlers: ClassVar[dict[type[TournamentCommand], Handler]] = {}
    _own_reaction_handlers: ClassVar[
        dict[type[TournamentProjectionEvent], Handler]
    ] = {}
    ...
    def _record_outcome(
        ...
        *,
        ...
        if (
            ...
            return []
        return [
            MatchOutcomeRecorded(phase_id=phase_id, snapshot=snapshot)
        ]
        ...
        **kwargs: Any,
        ...
        if profile.discipline_carryover is not None:
            for player_id, matches in profile.discipline_carryover.carryover(
                ...
                events.append(
                    SuspensionIssued(player_id=player_id, matches=matches)
                )
        ...
        view = PhaseStandingsView(profile.tiebreaker)

        if isinstance(ps, RoundRobinPhaseState):
            if ps.all_fixtures_resolved():
                quals = view.qualifiers(state, fact.phase_id, phase_def.qualification)
                events.append(
        ...
        if isinstance(ps, BracketPhaseState):
            ...
            for slot in bps.slots:
                if slot.contest_id == fact.snapshot.contest_id:
                    bps = replace_slot_winner(bps, slot.slot_id, fact.snapshot.winner_id)
                    break
