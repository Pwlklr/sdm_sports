...
def _has_official_override(match: Contest) -> bool:
    return any(isinstance(event, OfficialOverrideEvent) for event in match.history)


def _last_override_reason(match: Contest) -> str:
    for event in reversed(match.history):
...
def _winner_name_from_ranking(match: Contest) -> str:
...
def _format_football_played_outcome(state: FootballContestState) -> str:
...
def _print_archived_match(mid: str, match: Contest) -> None:
    state = match.current_state
    if isinstance(state, DartsContestState):
    ...
    if isinstance(state, FootballContestState):
        ...
        print(f"\nMatch: {desc} (ID: {mid[:8]})")
        outcome = _format_football_played_outcome(state)
        if _has_official_override(match):
            winner_name = _winner_name_from_ranking(match)
            print(f"Official Result: {winner_name} ({_last_override_reason(match)})")
            print(f"Played Result: {outcome}")
        else:
            print(f"Result: {outcome}")
        print("Final Score:")
        for team in state.teams:
            print(f"  - {team.name}: {state.scores[team.id]} goals")
        return
...
def _print_archived_tournaments(engine: SportsSystemEngine) -> None:
    print("\n--- Tournament History ---")
    ...
    for tournament in engine.tournaments.values():
        ...
        print(f"  Sport: {tournament.state.sport_id}")
        print(f"  Format: {tournament.state.blueprint_id}")
        print(f"  Status: {status}")
        champion_id: Optional[str] = tournament.state.champion_id
        if champion_id:
            champion_name = tournament.state.contestants.get(champion_id, champion_id)
            print(f"  Champion: {champion_name}")


def _print_archived_matches(engine: SportsSystemEngine) -> None:
    print("\n--- Match History (Archived) ---")
    if not engine.archived_matches:
        print("No matches have been completed yet.")
        return

    for mid, match in engine.archived_matches.items():
        _print_archived_match(mid, match)
    print("--------------------------------")
...
def main() -> None:
    ...
    while True:
        ...
        if choice == "1":
            ...
            _print_archived_matches(engine)
            _print_archived_tournaments(engine)
