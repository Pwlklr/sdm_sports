...
def test_draw_ranking_ex_aequo() -> None:
    ...
    concluded = MatchConcluded(winner_id=None, draw=True, decided_by="regulation")
    state = replace(state, scores={home.id: 1, away.id: 1})
...
def test_winner_ranking() -> None:
    ...
    builder = FootballResultBuilder(config=config)
    state = create_football_contest_state([home, away], config)
    concluded = MatchConcluded(winner_id=home.id, draw=False, decided_by="regulation")
    state = state.apply(concluded)
...
def test_build_official_overrides_team_scores_keeps_player_stats() -> None:
    ...
    concluded = MatchConcluded(winner_id=home.id, draw=False, decided_by="regulation")
    state = replace(state, player_stats=stats, scores={home.id: 1, away.id: 0})
    state = state.apply(concluded)
    ...
    result = builder.build_official(state, [concluded], override)
