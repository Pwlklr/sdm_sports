...
class DummyContestState(ContestState):
    ...
    @property
    def contestants(self) -> list:
        return []

    def apply(self, fact: Event) -> "DummyContestState":
        return self

    def reset(self) -> "DummyContestState":
        return self
...
def test_darts_leg_integrity_handler_reverses_leg_when_earlier_dart_revoked() -> None:
    ...
    target_dart = DartScored(
        event_id="dart_1", player_id="p1", sector=20, multiplier=2, points=40
    )
    winning_dart = DartScored(
        event_id="dart_2", player_id="p1", sector=20, multiplier=2, points=40
    )
