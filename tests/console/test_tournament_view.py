...
from src.console.tournament_view import (
    active_matches, standings_table, _format_final_status, schedule_view
)
...
def test_standings_table_with_data():
    phase = MagicMock(spec=GroupStagePhase)
    row1 = MagicMock()
    row1.points = 3; row1.wins = 1; row1.draws = 0; row1.losses = 0; row1.played = 1
    row1.contestant.name = "Team A"

    row2 = MagicMock()
    row2.points = 0; row2.wins = 0; row2.draws = 0; row2.losses = 1; row2.played = 1
...
def test_schedule_view():
    ...
    with patch('src.console.tournament_view._format_final_status', return_value="status"):
