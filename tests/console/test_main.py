...
from src.console.main import (
    main, print_menu, sport_id_for_match, select_sport, match_loop,
    setup_demo_roster, select_players, select_teams, select_contestants,
    play_tournament_match, _apply_suspension_context, _carry_over_discipline,
...
def test_select_contestants() -> None:
    engine = MagicMock()
    engine.list_individual_players.return_value = [IndividualPlayer("P1"), IndividualPlayer("P2"), IndividualPlayer("P3")]
...
def test_apply_suspension_context() -> None:
    ...
    state.with_tournament_context.assert_called_with(suspended_player_ids=frozenset(["1"]))
...
def test_run_tournament_matches(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with patch('src.console.main.active_matches', side_effect=[[], [], [], [], [MagicMock(spec=Contest)], []]):
        with patch('src.console.main.standings_table', return_value=["row"]):
            with patch('src.console.main.schedule_view', return_value=["sch"]):
                with patch('src.console.main.play_tournament_match') as mock_play:
                    with patch('builtins.input', side_effect=["invalid", "2", "3", "1", "1", "0", "4"]):
                        run_tournament_matches(MagicMock(), tournament, MagicMock())
                        out = capsys.readouterr().out
...
def test_main_choice_2(mock_match_loop, mock_create, mock_select_c, mock_select_s) -> None:
...
@patch('src.console.main.select_contestants')
@patch('src.console.main.create_console_contest')
def test_main_choice_2_invalid_config(mock_create, mock_select_c, mock_select_s) -> None:
    mock_sport = MagicMock()
    mock_select_s.return_value = mock_sport
    mock_select_c.return_value = [MagicMock(), MagicMock()]
...
def test_main_choice_3_group_complete(mock_create_tour, mock_run, mock_select_c, mock_select_s, mock_setup) -> None:
    mock_sport = MagicMock()
    mock_sport.adapter.collect_config.return_value = MagicMock()
    mock_select_s.return_value = mock_sport
    mock_select_c.return_value = [MagicMock(), MagicMock(), MagicMock()]
...
def test_main_choice_5() -> None:
    with patch('src.console.main.SportsSystemEngine') as MockEngine:
        engine = MockEngine.return_value
        engine.list_individual_players.return_value = [IndividualPlayer("P1", metadata={"nickname": "N1"})]
...
def test_main_choice_6(mock_sfp, mock_d2w) -> None:
    with patch('src.console.main.SportsSystemEngine') as MockEngine:
        ...
        engine.archived_matches = {
            "m1": darts, "m2": fb1, "m3": fb2, 
            "m4": fb3, "m5": fb4, "m6": fb5, "m7": gen
        }
